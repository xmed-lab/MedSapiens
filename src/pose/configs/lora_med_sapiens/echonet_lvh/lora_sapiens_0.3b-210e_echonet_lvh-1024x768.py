...
pretrained_checkpoint_lora="../../checkpoints/best_EPE_epoch_199.pth"
...
# hooks
default_hooks = dict(
    checkpoint=dict(save_best='EPE', rule='less', max_keep_ckpts=5),
...
model = dict(
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint=pretrained_checkpoint_lora,
    )
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                type='CoarseDropout',
                max_holes=1,
                max_height=0.15,  # at most 15% of image height
                max_width=0.15,  # at most 15% of image width
                min_holes=1,
                min_height=0.05,  # at least 5% of image height
                min_width=0.05,  # at least 5% of image width
                p=0.3,  # apply on 30% of images
            ),
        ]),
...
dataset_coco = dict(
    type='CocoDataset',
    data_root='../../data//echonetlvh/',
    data_mode='topdown',
    ann_file='../../data//train_new.json',
...
dataset_coco_val = dict(
    ...
    data_root='../../data/echonetlvh/',
    data_mode='topdown',
    ann_file='../../data/echonetlvh/val_new.json',
    data_prefix=dict(img='Images/'),
)
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        metainfo=dict(from_file='configs/_base_/datasets/echonet_lvh.py'),
...
val_evaluator = dict(
    type='EPE',
)  #ann_file='../data/leg/leg_coco_val_annotations.json')
test_evaluator = dict(
    type='EPE',
)  #ann_file='../data/leg/leg_coco_test_annotations.json')
